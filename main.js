...
function purgeSession() {
  let prekey = [];
  let directorio = readdirSync("./الجلسات");
...
function purgeOldFiles() {
  const directories = ["./الجلسات/", "./jadibts/"];
...
async function connectionUpdate(update) {
  ...
  if (reason == 405) {
    await fs.unlinkSync("./الجلسات/" + "creds.json");
    console.log(
      chalk.bold.redBright(
...
global.reloadHandler = async function (restatConn) {
  ...
  if (!isInit) {
    ...
    conn.sdemote = '[⚡]⌯ @user بقيت زيك زي اي حد هنا'
    ...
    conn.sAnnounceOn = '[⚡]⌯ البار اتفتح تع ارغي يحب'
    conn.sAnnounceOff = '[⚡]⌯ البار اتقفل يلا روح صلي'
    conn.sRestrictOn = '[⚡]⌯ الرول بس الي يقدرو يعدلو'
    conn.sRestrictOff = '[⚡]⌯ الكل بق يقدر يعدل ف البار'
